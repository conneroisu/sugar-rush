{
  inputs = let
    version = "1.4.1";
    system = "x86_64-linux";
    devenv_root = "/home/connerohnesorge/Documents/000Vaults/SecondBrain/.obsidian/plugins/sugar-rush";
    devenv_dotfile = ./.devenv;
    devenv_dotfile_string = ".devenv";
    container_name = null;
    devenv_tmpdir = "/run/user/1000";
    devenv_runtime = "/run/user/1000/devenv-9657905";
  ...
  outputs = {nixpkgs, ...} @ inputs: let
    ...
    build = options: config:
      ...
      (
        name: option:
          if builtins.hasAttr "type" option
          then
            if option.type.name == "output" || option.type.name == "outputOf"
            then {
              ${name} = config.${name};
            }
            else {}
